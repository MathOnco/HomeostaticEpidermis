...
## Checking over parameter space of the Second 2 parameters##
...
df <- read.csv("~/IdeaProjects/Epidermis_Project_Final/ParamFile_Iteration3.txt", sep = "\t", header = FALSE)
colnames(df) <- c("PSF", "EGF_CONS","APOPEGF","DEATHPROB","MOVE","DIVLOCPROB","rlambda","mean")
dists <- data.frame(matrix(NA, nrow=0, ncol=1))
colnames(dists) <- c("Dist")
for(i in 1:length(df$mean)){
...
## Checking over parameter space of the Second 6 parameters##
df <- read.csv("~/IdeaProjects/Epidermis_Project_Final/ParamFile_Iteration6.txt", sep = "\t", header = FALSE)
colnames(df) <- c("PSF", "EGF_CONS","APOPEGF","DEATHPROB","MOVE","DIVLOCPROB","rlambda","mean")
dists <- data.frame(matrix(NA, nrow=0, ncol=1))
colnames(dists) <- c("Dist")
for(i in 1:length(df$mean)){
...
subset(dfDist, dfDist$E.Dist==min(dfDist$E.Dist))

ggplot(dfDist, aes(DIVLOCPROB, rlambda)) + geom_point() + scale_x_log10()
