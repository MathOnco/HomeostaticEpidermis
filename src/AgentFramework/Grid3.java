...
/**
 * Extend the Grid2 class if you want a 3D lattice with one or more agents per grid voxel
 * @param <T> the AgentSQ3 or AgentPT3 extending agent class that will inhabit the grid
 */
public class Grid3 <T extends AgentSQ3> extends GridBase implements Iterable<T>{
    AgentList<T> agents;
     ...
     */
    public Grid3(int x,int y,int z, Class<T> type){
        xDim=x;
        yDim=y;
        zDim=z;
        length=x*y*z;
     ...
     */
    public int PopAtSq(int x, int y, int z){
        //gets population count at location
        return counts[SQtoI(x,y,z)];
    }
     ...
     */
    public T SQtoAgent(int x, int y, int z){
     ...
     */
    public void SQtoAgents(ArrayList<T>putHere, int x, int y, int z){
        T agent= SQtoAgent(x,y,z);
        while(agent!=null){
            putHere.add(agent);
