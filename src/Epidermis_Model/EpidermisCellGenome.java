...
public class EpidermisCellGenome extends GenomeInfo <EpidermisCellGenome> {
    ...
    private static long[][][] ParseBaseIndexes(){
        ...
        long[][][] BaseIndexes = new long[data.size()/4][5][];
        for (int i = 0; i < data.size()/4; i+=4) {
            BaseIndexes[i][0] = data.get(i);
            BaseIndexes[i][1] = data.get(i+1);
            BaseIndexes[i][2] = data.get(i+2);
            BaseIndexes[i][3] = data.get(i+3);
        }
        return BaseIndexes;
    }
}
