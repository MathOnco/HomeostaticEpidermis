...
//            }
            ...
            EpidermisGrid Epidermis = new EpidermisGrid(EpidermisConst.xSize, EpidermisConst.ySize, EpidermisConst.zSize); // Initializes and sets up the program for running
            ...
            while (Done) {
                ...
                if (Epidermis.GetTick() > 1) {
                    FixTest = Epidermis.CheckFixation();
                    EndRun = FixTest.GetFreq();
             ...
             */
                if (EpidermisConst.get_r_lambda) {
                    if (Epidermis.GetTick() % 7f == 0) {
                        ...
                        r_lambda_index += 1;
                        meanCellAge.add(meanCellAgeIndex, Epidermis.GetMeanCellAge(Epidermis));
                        meanCellAgeIndex += 1;
                        if (rLambda_Label != null) {
                            rLambda_Label.setText("Mean rLambda: " + new DecimalFormat("#.000").format((Epidermis.r_lambda_weekly / (EpidermisConst.xSize * EpidermisConst.zSize)) / 7f));
                        }
...
//                }
             ...
             */
                if (ActivityVis != null) {
                    YearLab.setText("Age (yrs.): " + new DecimalFormat("#.00").format((Epidermis.GetTick() / 365f)));
                }
                if (DivVis != null & Epidermis.GetTick() % EpidermisConst.VisUpdate == 0) {
                    Epidermis.ActivityHeatMap(DivVis, Epidermis, CellDraw, Epidermis.MeanProlif, "gbr");
                }
                if (DivLayerVis != null & Epidermis.GetTick() % EpidermisConst.VisUpdate == 0) {
                    Epidermis.LayerVis(DivLayerVis, Epidermis, CellDraw, Epidermis.MeanProlif, "gbr");
                }
                if (DeathVis != null & Epidermis.GetTick() % EpidermisConst.VisUpdate == 0) {
                    Epidermis.ActivityHeatMap(DeathVis, Epidermis, CellDraw, Epidermis.MeanDeath, "rbg");
                }
                if (DeathLayerVis != null & Epidermis.GetTick() % EpidermisConst.VisUpdate == 0) {
                    Epidermis.LayerVis(DeathLayerVis, Epidermis, CellDraw, Epidermis.MeanDeath, "rbg");
                }
                if (ClonalVis != null) {
                    Epidermis.DrawCellPops(ClonalVis, Epidermis, CellDraw);
                } // 3D Good
                if (OldestCell != null) {
                    OldestCell.setText("Mean cell age: " + new DecimalFormat("#.00").format(Epidermis.GetMeanCellAge(Epidermis)));
                }
                if (ActivityVis != null) {
                    Epidermis.DrawCellActivity(ActivityVis, Epidermis, CellDraw);
                }
                if (BottomVis != null) {
                    Epidermis.DrawCellPopsBottom(BottomVis, Epidermis, CellDraw);
                }
                if (BottomVisMove != null) {
                    Epidermis.DrawCellPopsBottomActivity(BottomVisMove, Epidermis, CellDraw);
                }
                if (EGFVis != null) {
                    Epidermis.DrawChemicals(EGFVis, true, false);
                } // 3D Good
                if (Epidermis.GetTick() % 10 == 0) {
//                avgHeight=(Epidermis.popSum*1.0/Epidermis.GetTick())/Epidermis.xDim/Epidermis.zDim;
                    avgHeight = Epidermis.GetMeanCellHeight(Epidermis);
                    if (HeightLab != null) {
                        HeightLab.setText("Height: " + new DecimalFormat("#.00").format(avgHeight));
                    }
                }
...
//            }
        ...
        System.out.println(d + "\t" + 0.3 + "\t" + FixedTime + "\t" + Epidermis.GetMeanCellAge(Epidermis) + "\t" + Epidermis.GetMeanCellHeight(Epidermis) + "\t" +(r_lambOut/r_lambda_index));
