...
public class LossReplace {
    ...
    public double RecordTissueRate(String Option){
        double Rate=0.0;
        if(Option=="Birth"){
            Rate=(DivisionTissue*1.0)/ (theGrid.xDim); // Gives a per day rate
            BirthRateTissue[theGrid.GetTick()] = Rate;
        } else if(Option=="Death"){
            Rate=(DeathTissue*1.0)/ (theGrid.xDim); // Gives a per day rate
